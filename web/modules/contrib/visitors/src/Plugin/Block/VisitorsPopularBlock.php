 ...
 * )
 */
class VisitorsPopularBlock extends BlockBase implements ContainerFactoryPluginInterface {
   ...
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, EntityTypeManagerInterface $entity_type_manager, EntityRepositoryInterface $entity_repository, VisitorsCounterInterface $statistics_storage, RendererInterface $renderer) {
  ...
  public function build() {
    ...
    if ($this->configuration['top_day_num'] > 0) {
      $nids = $this->statisticsStorage->fetchAll('today', $this->configuration['top_day_num']);
      if ($nids) {
    ...
    if ($this->configuration['top_all_num'] > 0) {
      $nids = $this->statisticsStorage->fetchAll('total', $this->configuration['top_all_num']);
      if ($nids) {
